...
def Tree_visus(table_names, type_name):
    if type_name != 'SELECT':
        ...
        c = (
            ...
            .add("", data)
            .set_global_opts(
                title_opts=opts.TitleOpts(title="血缘-{}".format(type_name)),
                toolbox_opts=opts.ToolboxOpts(is_show=True),  # 添加工具箱
                tooltip_opts=opts.TooltipOpts(trigger="item", trigger_on="mousemove")  # 添加提示框
            )
        )
        ...
        c = (
            ...
            .add(
                ...
                data,
                orient="TB",  # 树的方向：从上到下
                initial_tree_depth=2  # 初始展开深度
            )
            .set_global_opts(
                title_opts=opts.TitleOpts(title="查询-{}".format(type_name)),
